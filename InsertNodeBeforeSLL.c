...
void insertNodeBefore(int beforeData,int data)
{
    LinkedList* newNode = createNode(data);
    LinkedList* temp = first;
    if(first->data==beforeData)
    {
        newNode->next = first;
        first = newNode;
    }
    ...
    {
        while(temp!=NULL && temp->next->data!=beforeData )
        {
            temp = temp->next;
        }
    ...
    do
    {
        ...
        switch(choice)
        {
            ...
            case 2:
                ...
                scanf("%d",&data);
                insertNodeBefore(beforeData,data);
                break;
            case 3:
                display();
                break;
